...
import {
  ...
  GovernanceMock,
  TreasuryMock,
} from "@/components/work/orbitx-mocks";
import { OrbitxEscrow } from "@/components/work/orbitx-escrow";
import {
  SsiMock,
  OnChainTrustMock,
  FraudDetectionMock,
} from "@/components/work/sedax-mocks";
...
function parseMarkdown(content: string) {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    if (text.startsWith("Direct Answer"))
      directAnswer = text.replace(/^Direct Answer\n+/, "");
    else if (text.startsWith("What Metaborong Built"))
      solution = text.replace(/^What Metaborong Built\n+/, "");
    else if (text.startsWith("Results"))
      results = text.replace(/^Results\n+/, "");
    else if (text.startsWith("Frequently Asked Questions"))
      faq = text.replace(/^Frequently Asked Questions\n+/, "");
    else if (text.startsWith("Technical Approach"))
      techApproach = text.replace(/^Technical Approach\n+/, "");
  }
...
export default async function CaseStudyPage({
  ...
  if (slug === "sunset")
    return <SunsetCaseStudy meta={meta} slug={slug} parsed={parsed} />;
  if (slug === "magic")
    return <MagicCaseStudy meta={meta} slug={slug} parsed={parsed} />;
  if (slug === "orbitx")
    return <OrbitxCaseStudy meta={meta} slug={slug} parsed={parsed} />;
  if (slug === "sedax")
    return <SedaxCaseStudy meta={meta} slug={slug} parsed={parsed} />;
...
function SunsetCaseStudy({
  ...
  const solutionFeatures = parsed.solutionFeatures.map((f) => ({
    ...f,
    body: stripTrailingRule(f.body),
  }));
    ...
    <main className="flex min-h-screen flex-col">
      ...
      {directAnswer && (
        <Section
          bg="default"
          maxWidth="xwide"
          className="pt-[48px] sm:pt-[72px] lg:pt-[100px] pb-[32px] sm:pb-[48px] lg:pb-[60px]"
        >
          <div className="rounded-[16px] border border-border bg-bg-subtle p-[24px] sm:p-[32px] lg:p-[40px]">
            ...
            <span className="mt-[18px] block font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em] text-[#c43a00]">
              In short · What is {meta.client ?? "SunsetML"}?
            </span>
            <p className="mt-[14px] max-w-[46ch] text-[clamp(19px,2.3vw,28px)] font-bold tracking-[-0.025em] leading-[1.25] text-dark">
              {meta.client ?? "SunsetML"} is an AI-native writing platform that
              embeds multi-model AI inside the editor, so writers research,
              draft, and refine without losing context.
            </p>
            <p className="mt-[14px] max-w-[60ch] text-[15px] sm:text-[16px] leading-[1.6] text-gray">
              Designed and engineered from scratch by Metaborong, as engineering
              partner and equity co-founder. Writers switch live across four
              frontier models without breaking flow.
            </p>
            <div className="mt-[22px] flex flex-wrap gap-[8px]">
              {[
                "Contextual PromptBar",
                ...
                <span
                  key={c}
                  className="border border-border px-[10px] py-[5px] text-[13px] font-semibold text-gray"
                >
                  {c}
                </span>
              ))}
      ...
      {techApproach && (
        <Section
          bg="default"
          maxWidth="xwide"
          className="py-[48px] sm:py-[72px] lg:py-[100px] border-t border-border"
        >
          <SunsetTech />
        </Section>
      )}
      ...
      <WorkRelatedServices
        related={related}
        title="How we build platforms like SunsetML"
        accent="#c43a00"
      />
...
function MagicCaseStudy({
  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      {directAnswer && (
        ...
        >
          <div className="rounded-[16px] border border-border bg-bg-subtle p-[24px] sm:p-[32px] lg:p-[40px]">
            ...
            <span className="mt-[18px] block font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em] text-[#6d28d9]">
              In short · What is MAGIC?
            </span>
            <p className="mt-[14px] max-w-[52ch] text-[clamp(19px,2.3vw,28px)] font-bold tracking-[-0.025em] leading-[1.25] text-dark">
              MAGIC is an AI creative automation platform that turns a single
              product image into product videos, CGI scene visuals, and
              marketplace packshots, generated automatically and at scale
              without a photography studio.
            </p>
            <p className="mt-[14px] max-w-[60ch] text-[15px] sm:text-[16px] leading-[1.6] text-gray">
              Built ground-up by Metaborong as engineering partner for{" "}
              <a
                href="https://omagic.ai/"
                target="_blank"
                rel="noopener"
                className="font-semibold text-[#6d28d9] underline decoration-[#6d28d9]/30 underline-offset-2 hover:decoration-[#6d28d9]"
              >
                Omagic AI
              </a>
              , using controlled generation pipelines built for brand
              consistency and production reliability across Amazon, Flipkart,
      ...
      <WorkRelatedServices
        related={related}
        title="How we build platforms like MAGIC"
        accent="#6d28d9"
      />
...
const ORBITX_TECH_NODES: readonly TechNode[] = [
  {
    ...
    tech: "USDC in and out",
    note: "fiat and crypto workflows",
  },
  {
    n: "02",
    name: "Smart contract core",
    tech: "UUPS proxy · RBAC",
    note: "upgradeable, permissioned",
    chips: [
      "Upgrade safety",
      "Timelock delays",
      "Multi-sig ops",
  ...
  {
    n: "03",
    name: "Treasury and yield",
    tech: "Aave · Morpho on Base",
    note: "yield on idle reserves",
  },
  {
    n: "04",
    name: "Settlement",
    chips: ["Payments", "Escrow", "Treasury", "Cross-border"],
    chipsActiveFirst: true,
  },
];

const ORBITX_TECH_INTRO =
  "OrbitX runs as one cohesive banking protocol on Coinbase's Base network: an upgradeable contract core gated by timelock governance and role-based access, a USDC treasury earning DeFi yield, and an on-chain escrow that settles against milestones, all sharing one secure architecture.";

function OrbitxCaseStudy({
  meta,
  slug,
  parsed,
}: {
  ...
  const mocks = [
    <UpgradeableMock key="u" />,
    <GovernanceMock key="g" />,
    <TreasuryMock key="t" />,
    <OrbitxEscrow key="e" />,
  ];
  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      {directAnswer && (
        ...
        >
          <div className="rounded-[16px] border border-border bg-bg-subtle p-[24px] sm:p-[32px] lg:p-[40px]">
            ...
            <span className="mt-[18px] block font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em] text-[#0e7490]">
              In short · What is OrbitX?
            </span>
            <p className="mt-[14px] max-w-[54ch] text-[clamp(19px,2.3vw,28px)] font-bold tracking-[-0.025em] leading-[1.25] text-dark">
              OrbitX is a stablecoin banking infrastructure platform that lets
              businesses spend, send, receive, and manage USDC-linked funds
              globally through smart contracts, treasury systems, and on-chain
              settlement.
            </p>
            <p className="mt-[14px] max-w-[62ch] text-[15px] sm:text-[16px] leading-[1.6] text-gray">
              Re-architected from an early MVP into production-grade banking
              infrastructure by Metaborong as development partner, on
              Coinbase&apos;s Base network.{" "}
              <a
                href="https://orbitxpay.com/"
                target="_blank"
                rel="noopener"
                className="font-semibold text-[#0e7490] underline decoration-[#0e7490]/30 underline-offset-2 hover:decoration-[#0e7490]"
              >
                OrbitX
              </a>{" "}
      ...
      {techApproach && (
        ...
        >
          <WorkTech
            intro={ORBITX_TECH_INTRO}
            nodes={ORBITX_TECH_NODES}
            accent="#0e7490"
          />
        </Section>
      )}
      ...
      <WorkRelatedServices
        related={related}
        title="How we build platforms like OrbitX"
        accent="#0e7490"
      />
...
const SEDAX_TECH_NODES: readonly TechNode[] = [
  {
    n: "01",
    name: "Identity claim",
    tech: "Held in user wallet",
    note: "W3C Verifiable Credentials",
  },
  {
    n: "02",
    name: "Zero-Knowledge Proof",
    tech: "Prove without revealing",
    note: "no raw data transmitted",
    chips: ["Age", "Nationality", "Credential", "Liveness"],
  },
  {
    n: "03",
    name: "On-chain trust",
    tech: "Issue · verify · revoke",
    note: "tamper-evident ledger",
  },
  {
    n: "04",
    name: "Verified",
    chips: ["Reusable KYC", "KYC/AML audit", "GDPR · DPDP", "No PII stored"],
    chipsActiveFirst: true,
  },
];

const SEDAX_TECH_INTRO =
  "SEDAX verifies identity across three layers: claims held in a user-controlled wallet as W3C Verifiable Credentials, a Zero-Knowledge Proof core that confirms a claim without transmitting the underlying data, and a tamper-evident ledger recording issuance, verification, and revocation, with AI-assisted liveness and document checks guarding onboarding.";

function SedaxCaseStudy({
  meta,
  slug,
  parsed,
}: {
  ...
  const mocks = [
    <SedaxZkp key="z" />,
    <SsiMock key="s" />,
    <OnChainTrustMock key="t" />,
    <FraudDetectionMock key="f" />,
  ];
  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      {directAnswer && (
        ...
        >
          <div className="rounded-[16px] border border-border bg-bg-subtle p-[24px] sm:p-[32px] lg:p-[40px]">
            ...
            <span className="mt-[18px] block font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em] text-[#3254fb]">
              In short · What is SEDAX?
            </span>
            <p className="mt-[14px] max-w-[54ch] text-[clamp(19px,2.3vw,28px)] font-bold tracking-[-0.025em] leading-[1.25] text-dark">
              SEDAX is a blockchain eKYC platform that verifies identity with
              Zero-Knowledge Proofs, so users prove who they are without sharing
              the underlying personal data.
            </p>
            <p className="mt-[14px] max-w-[62ch] text-[15px] sm:text-[16px] leading-[1.6] text-gray">
              Built from the ground up by Metaborong as development partner on
              self-sovereign identity and W3C Verifiable Credentials.{" "}
              <a
                href="https://www.sedax.in/"
                target="_blank"
                rel="noopener"
                className="font-semibold text-[#3254fb] underline decoration-[#3254fb]/30 underline-offset-2 hover:decoration-[#3254fb]"
              >
                SEDAX
              </a>{" "}
              replaces centralized document collection with privacy-first,
      ...
      {techApproach && (
        ...
        >
          <WorkTech
            intro={SEDAX_TECH_INTRO}
            nodes={SEDAX_TECH_NODES}
            accent="#3254fb"
          />
        </Section>
      )}
      ...
      <WorkRelatedServices
        related={related}
        title="How we build platforms like SEDAX"
        accent="#3254fb"
      />
...
function WorkHero({ meta }: { meta: CaseStudyMeta }) {
  ...
  const titleSuffix =
    titleSuffixParts.length > 0 ? titleSuffixParts.join(":").trim() : null;

  // Theme control parameters
  const isLight = meta.lightTheme === true;
  const bgStyle = { backgroundColor: meta.bgColor ?? "#0a0a0a" };

  return (
    /* 
      ==========================================================================
      ACTIVE WORKHERO: SEPARATED BANNER + THEME CONTROLS (TESTING PHASE)
      --------------------------------------------------------------------------
      - Background color customizable via `meta.bgColor` (e.g. #ffffff)
      - Text / border colors automatically adapt using `meta.lightTheme` (boolean)
      - The banner is a separate box above the text (so text does not overlay it)
      ==========================================================================
    */
    <section
      className="relative flex flex-col overflow-hidden pt-[100px] sm:pt-[120px] pb-[48px] sm:pb-[64px] lg:pb-[80px] transition-colors duration-300"
      style={bgStyle}
    >
      <div className="mx-auto w-full max-w-7xl px-[16px] sm:px-[24px] md:px-[48px] lg:px-[80px] xl:px-[128px] flex flex-col">
        
        {/* Breadcrumb / Back Link */}
        <div className="mb-[16px]">
          <Link
            href="/#work"
            className={`group -my-[14px] inline-flex items-center gap-[7px] py-[14px] font-mono text-[11px] font-bold uppercase tracking-[0.08em] transition-colors duration-[var(--duration-fast)] ${
              isLight ? "text-black/65 hover:text-black" : "text-white/65 hover:text-white"
            }`}
          >
            <ArrowLeft
              size={13}
            ...
            <span className="relative inline-block">
              ...
              <span
                aria-hidden
                className={`absolute -bottom-[3px] left-0 h-[1px] w-full origin-left scale-x-0 transition-transform duration-[var(--duration-base)] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-x-100 ${
        ...
        {/* Cinematic Video Banner Container (Renders separate from and above the text flow) */}
        {meta.bannerLogo && (
          <div className="relative w-full h-[25vh] min-h-[180px] sm:min-h-[260px] lg:min-h-[380px] max-h-[480px] rounded-[16px] sm:rounded-[24px] overflow-hidden bg-white/5 mb-[24px] sm:mb-[32px] border border-white/10 shadow-lg">
            {meta.animatedLogo ? (
              <video
                ...
                className={`relative w-full h-full object-cover object-center ${
                  meta.blendLogo ? " mix-blend-screen" : ""
                }`}
              />
            ) : (
              <div className="relative w-full h-full flex items-center justify-center">
                <img
                  src={meta.logo}
                  alt=""
                  className={`h-[40px] sm:h-[64px] object-contain ${
                    isLight ? "opacity-90" : "invert opacity-80"
                  }`}
                />
              </div>
            )}
        ...
        {/* Text Section (Client Tag, Title, Metadata) */}
        <div className="relative z-10 w-full">
          {/* Eyebrow Details */}
          {meta.bannerLogo && (
            <div className="mb-[12px] sm:mb-[16px] flex items-center gap-[10px]">
              <span
                className="font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em]"
                style={{ color: meta.glowColor ?? (isLight ? "#000000" : "#3b5bff") }}
              >
                {client}
              </span>
              <span className={isLight ? "text-black/30" : "text-white/30"}>•</span>
              <span className={`font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] ${
          ...
          {titleSuffix ? (
            <h2
              className={`text-[clamp(24px,3.5vw,48px)] font-medium tracking-[-0.02em] leading-[1.1] pr-[16px] ${
                isLight ? "text-black/80" : "text-white/80"
              } ${publishedLabel ? "mb-[16px] sm:mb-[24px]" : "mb-[24px] sm:mb-[32px]"}`}
            >
              {titleSuffix}
            </h2>
          ) : (
            <h1
              className={`text-[clamp(28px,5vw,80px)] font-bold tracking-[-0.04em] leading-[1.0] pr-[16px] ${
                isLight ? "text-black" : "text-white"
              } ${publishedLabel ? "mb-[24px] sm:mb-[32px]" : "mb-[40px] sm:mb-[56px]"}`}
            >
              {meta.title}
            </h1>
          )}
          ...
          {publishedLabel && (
            <p className={`mb-[24px] sm:mb-[32px] text-[13px] sm:text-[14px] font-medium ${
              isLight ? "text-black/55" : "text-white/55"
            }`}>
              By Metaborong, {meta.bylineRole ?? "engineering partner and equity co-founder"}
              <span className={isLight ? "mx-[8px] text-black/30" : "mx-[8px] text-white/30"}>·</span>
              Published {publishedLabel}
            </p>
          )}

          {/* Metadata Grid */}
          <div className={`flex flex-wrap gap-x-[32px] sm:gap-x-[48px] gap-y-[20px] border-t pt-[28px] sm:pt-[36px] ${
            ...
            {[
              ...
              {
                label: "Services",
                value: meta.services ?? "Platform Engineering",
              },
              { label: "Year", value: meta.year ?? "2024" },
            ].map((item) => (
              <div key={item.label} className="flex flex-col gap-[6px]">
                <span className={`font-mono text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] ${
                  isLight ? "text-black/55" : "text-white/55"
                }`}>
                  {item.label}
                </span>
...
/*
  =============================================================================
  PROVISION FOR ROLLBACK: PREVIOUS FULL-BLEED WORKHERO LAYOUT
  -----------------------------------------------------------------------------
  To roll back completely, delete the active WorkHero component above,
  uncomment the entire block below, and remove the `/*` and `*\/` wrappers.
  =============================================================================
...
function WorkHero({ meta }: { meta: CaseStudyMeta }) {
  ...
  return (
    <section
      className={`relative flex flex-col justify-end overflow-hidden bg-[#0a0a0a] ${
        meta.bannerLogo
          ? "min-h-[80svh] md:min-h-[90svh] pt-[120px]"
          : "min-h-[100svh] pt-[100px] sm:pt-[120px] lg:pt-[160px]"
      }`}
    >
      <div className="absolute inset-0 z-0 pointer-events-none" aria-hidden>
        {meta.bannerLogo ? (
          <>
            {meta.animatedLogo ? (
              <video
                ...
                muted
                playsInline
                className={`w-full h-full object-cover object-center opacity-30 md:opacity-45 transition-opacity duration-1000${
                  meta.blendLogo ? " mix-blend-screen" : ""
                }`}
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-white/5 opacity-20">
                <img
                  src={meta.logo}
                  alt=""
                  className="h-[60px] sm:h-[80px] object-contain invert"
                />
              </div>
            )}

            <div className="absolute inset-0 bg-linear-to-t from-[#0a0a0a] via-[#0a0a0a]/65 to-[#0a0a0a]/20 z-10" />
            <div className="absolute inset-0 bg-linear-to-r from-[#0a0a0a] via-[#0a0a0a]/50 to-transparent z-10" />

            <div
              className="absolute bottom-[10%] left-[-20%] right-[-20%] h-[80vh] mix-blend-screen opacity-30 pointer-events-none z-10"
              style={{
          ...
          <>
            ...
            <div className="absolute bottom-0 inset-x-0 h-[35%] bg-linear-to-t from-[#0a0a0a] to-transparent" />
          </>
        )}
      </div>
            ...
            {meta.animatedLogo ? (
              ...
              <div className="relative w-full h-full flex items-center justify-center transition-transform duration-[400ms] ease-out group-hover:scale-105">
                <img
                  src={meta.logo}
                  alt=""
                  className="h-[40px] sm:h-[56px] object-contain invert opacity-80"
                />
              </div>
            )}
          </a>
      ...
      <div className="relative z-10 mx-auto w-full sm:px-[16px] md:px-[40px] lg:px-[48px] xl:px-[80px] 2xl:px-[128px] flex flex-col flex-1 justify-end pb-[48px] sm:pb-[64px] lg:pb-[80px]">
        <div className="mb-[16px]">
          <Link
            href="/#work"
        ...
        {meta.bannerLogo && (
          <div className="mb-[12px] sm:mb-[16px] flex items-center gap-[10px]">
            <span
              className="font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em]"
              style={{ color: meta.glowColor ?? "#3b5bff" }}
            >
              {client}
            </span>
            <span className="text-white/30">•</span>
            <span className="font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] text-white/50">
              {meta.category}
            </span>
          </div>
        )}
        ...
        <div className={`relative z-10 ${meta.bannerLogo ? "mt-auto" : ""}`}>
          {titleSuffix ? (
            <h2
              className={`text-[clamp(24px,3.5vw,48px)] font-medium tracking-[-0.02em] leading-[1.1] text-white/80 ${meta.bannerLogo ? "max-w-full" : "max-w-[min(620px,70vw)] sm:max-w-[min(700px,72vw)] lg:max-w-[900px]"} pr-[16px] ${publishedLabel ? "mb-[16px] sm:mb-[24px]" : "mb-[24px] sm:mb-[32px]"}`}
            >
              {titleSuffix}
            </h2>
          ) : (
            <h1
              className={`text-[clamp(28px,5vw,80px)] font-bold tracking-[-0.04em] leading-[1.0] text-white pr-[16px] ${meta.bannerLogo ? "max-w-full" : "max-w-[min(620px,70vw)] sm:max-w-[min(700px,72vw)] lg:max-w-[900px]"} ${publishedLabel ? "mb-[24px] sm:mb-[32px]" : "mb-[40px] sm:mb-[56px]"}`}
            >
              {meta.title}
            </h1>
          )}
          {publishedLabel && (
            <p className="mb-[24px] sm:mb-[32px] text-[13px] sm:text-[14px] font-medium text-white/55">
              By Metaborong, {meta.bylineRole ?? "engineering partner and equity co-founder"}
          ...
          <div className="flex flex-wrap gap-x-[32px] sm:gap-x-[48px] gap-y-[20px] border-t border-white/10 pt-[28px] sm:pt-[36px]">
            {[
              ...
              <div key={item.label} className="flex flex-col gap-[6px]">
                <span className="font-mono text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-white/55">
                  {item.label}
                </span>
                <span className="text-[13px] sm:text-[15px] font-medium text-white/80">
                  {item.value}
                </span>
              </div>
            ))}
...
function DemoVideo({ meta, slug }: { meta: CaseStudyMeta; slug: string }) {
  return (
    ...
    >
      <div
        className={`w-full aspect-[16/8] rounded-[12px] sm:rounded-[20px] lg:rounded-[24px] bg-canvas shadow-xl flex items-center justify-center relative overflow-hidden${meta.demoVideo ? "" : " group cursor-pointer"}`}
      >
        {meta.demoVideo ? (
          <WorkDemoVideo
            src={meta.demoVideo}
            poster={meta.demoPoster}
            label={`${meta.appName ?? meta.client ?? meta.title.split(":")[0]} product demo`}
          />
        ) : (
          <>
            ...
            <div className="absolute bottom-[16px] sm:bottom-[24px] left-[16px] sm:left-[24px] right-[16px] sm:right-[24px] flex justify-between items-center text-white/50 font-mono text-[10px] sm:text-[11px] font-bold tracking-[0.14em] uppercase z-10">
              <span className="flex items-center gap-[8px]">
                <span className="w-[6px] h-[6px] bg-red-500 rounded-full animate-pulse" />
                {slug} Demo
              </span>
...
function resolveRelated(
  pillarId: string,
  slugs: string[],
): { pillar: Pillar; leaf: ChildService }[] {
  const pillar = pillars.find((p) => p.id === pillarId);
  if (!pillar) return [];
  const leaves = getAllLeaves(pillar);
  return slugs
    .map((slug) =>
      leaves.find((l) => l.slug === slug && l.status === "published"),
    )
    .filter((l): l is ChildService => Boolean(l))
    .map((leaf) => ({ pillar, leaf }));
}

function WorkRelatedServices({
  related,
  title,
  accent,
}: {
  related: { pillar: Pillar; leaf: ChildService }[];
  title: string;
  accent: string;
}) {
  ...
  return (
    ...
    >
      <div className="mb-[32px] sm:mb-[40px]">
        <h2
          className="mb-[16px] font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em]"
          style={{ color: accent }}
        >
          Related services
        </h2>
      ...
      <ul
        ...
        style={{ "--rs-accent": accent } as React.CSSProperties}
      >
          ...
          <li key={leaf.slug} className="border-b border-border">
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-[4px] max-w-[68ch] text-[14px] leading-[1.55] text-gray">
                  {leaf.description}
                </p>
              </div>
              <span className="flex shrink-0 items-center justify-center text-gray transition-[transform,color] duration-[var(--duration-fast)] group-hover:translate-x-[6px] group-hover:text-[var(--rs-accent)] motion-reduce:group-hover:translate-x-0 mr-2">
                <span
                  aria-hidden="true"
                  className="text-[22px] sm:text-[26px] md:text-[30px] font-light leading-none"
                >
                  →
                </span>
              </span>
...
function buildWorkJsonLd(
  ...
  const article: Record<string, unknown> = {
    ...
    about: {
      ...
      name: appName,
      applicationCategory: meta.appCategory ?? "Software",
    },
  };
