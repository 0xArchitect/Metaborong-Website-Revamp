...
- [2026-05-18] **Visual structure superseded by `docs/superpowers/specs/2026-05-18-section-hero-fullbleed-design.md`.** The hero is now a single full-bleed **static landscape image** stage (`public/hero-bg.jpg` via `next/image`), off-white copy over a left-anchored dark scrim, the three glass cards locked onto the image's flower cluster. New deviations **D1–D3** (full-bleed decorative imagery; on-dark copy on a light site; left-anchored scrim gradient) are recorded in that spec + CHANGELOG. **§1a (ASCII shimmer/turbulence infinite — and the later ASCII *video* it became) is retired entirely** — the background no longer animates; this is a net compliance gain (one fewer infinite, not a swap). An in-session live-ASCII-canvas variant was built and then **reverted at user direction** in favor of the static image (no D4). **§1b (glassmorphic card loading/result cycle) is unchanged.** Supersedes the 2026-05-04 "Hero unchanged" memory lock (user re-opened explicitly). Historical §1a/§1b text above is left intact per the no-contradicting-versions rule — this note is the authoritative current state.
