...
- **Work-Preview production status board** (`work-preview.tsx`, 2026-05-27): the four projects
  are a single bordered **status-board manifest**, not cards. A header bar (`PRODUCTION` /
  `● {n} live`, mono, `bg-bg-subtle`) tops one `border border-border` container; each project is
  a hairline-divided row. The lone live token sits in the header — **not repeated per row**.
  - **lg+**: row is a 3-col grid `grid-cols-[150px_minmax(0,1fr)_auto]` (logo / category +
    title / `READ CASE STUDY →`), vertically centered. Hover = `--color-bg-raised` tint
    (bg-color transition, 200ms, `motion-reduce` neutralized). Category is the
    **pillar-colored mono tag** (`text-ai` for AI rows, `text-brand` for Web3); titles stay
    Switzer for readability. Live dot is **brand blue** (the site signal color — no untokened
    success-green introduced).
  - **Single-viewport (lg+ only):** the `<Section>` is gated to a `min-h-[calc(100svh-56px)]`
    centered flex frame with an svh-clamp padding override (`lg:py-[clamp(24px,4svh,72px)]!`) +
    `[&>div]:w-full` — measured fit 712/744/844 against the 712/744/844 frame at 1024/1280/1440.
  - **< lg:** the same `<a>` rows reflow to stacked cards (normal flow, `Section` default
    64/96 padding) — logo, then category + title, then Read. **One DOM tree, no duplicate
    markup** (mirrors the Comparison/Why-Us responsive idiom; SSR-crawlable, no a11y dupes).
  Replaced the 2026-05-24 hairline-seam 4-up card grid + the `<lg` drag-scroll snap lane (the
  prior "strip"). `--color-bg-raised` (`#fafbff`) still consumed here.
...
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| 2026-06-09 | **`/work/sunset` case study re-architected to an AEO section order (SunsetML).** First case study migrated off the legacy shared markdown pipeline to a question-driven layout: Hero (signature dark hero + animated logo + warm `#ff6b35` bloom kept) → **"In short" direct answer** (citable ≤150w, `In short / What is SunsetML?` 4fr/6fr split, AEO answer leads the body) → standalone Problem → **demo video moved up** → numbered "What Metaborong Built" narrative spine (sticky left rail + prose, *not* a card grid) → **prose** Technical Approach → dark Results band (real metrics only) → tech marquee → **shared `FaqSection`** (swapped off the inline `<details>`) → new **Related-services hairline list** (boxless rule-separated rows linking published `/services/ai/*` leaves) → CTA. Built as a per-slug `SunsetCaseStudy` branch in `app/work/[slug]/page.tsx`; magic/orbitx/sedax stay **byte-identical** on `LegacyCaseStudy` (shared blocks extracted with identical classNames; verified). Copy ran the full A3 SEO chain (keyword research → landing-page-writer → 3 parallel auditors: content-quality 94, E-E-A-T 8.5/10, GEO ~91) with **zero fabrication** — every metric (350 waitlist, 75 users, GPT-5.5/5.4-mini + Claude Sonnet 4.6/Opus 4.7, Next.js/React/NestJS/Framer Motion) user-supplied; Gemini removed as unconfirmed. **SEO plumbing added (work pages had none):** Article + BreadcrumbList + FAQPage JSON-LD on the sunset path, generic `/work/[slug]/raw.md` GEO export (all 4) + `text/markdown` alternate, `/work/*` added to sitemap; shared metadata in `lib/work.ts`. impeccable: detector clean (`[]`), critique ~35/40 after fixing a P0 (prose Technical Approach was hitting the legacy bullet-grid → empty card; now rendered via `<Markdown>`, trailing `---` stripped). Owner shipped the hard dark/light seams + autoplay demo as-is (both touch shared blocks → deferred to the all-4 migration). Pre-existing brand-token bypasses (`#3b5bff` hero/Results bloom, intentional `#ff6b35` glow) carried over verbatim, not introduced. `tsc`/lint clean (one pre-existing hero-logo `<img>` warning). | Establishes the AEO case-study grammar: a citable direct answer in the first screen, every section a self-contained answer to one real search query, and machine-readable structure (JSON-LD + raw.md) the legacy work pages lacked — without re-templating into the `/services` leaf shell (keeps the distinctive case-study hero/demo signature) and without disturbing the three case studies awaiting their own pass. |
| 2026-06-09 | **SunsetML — Technical Approach routing-pipeline + warm accent ramp + audit/perf fixes (impeccable live overdrive + audit/critique).** `/work/sunset` Technical Approach rebuilt from prose into a **routing pipeline** (Editor surface → NestJS orchestration → model providers) on a vertical spine with a reduced-motion-safe animated signal pulse, the four concurrency constraints tagged on the routing node; no cards. New `components/work/sunset-tech.tsx` (chosen via `/impeccable live` overdrive, variant 1 of 3). **Warm accent (sunset-scoped, accessible ramp):** the Results band now uses the **SunsetML hero glow `#ff6b35`** (bloom + bullet dots) so it bookends the hero; functional warm (Problem eyebrow + the shared `ContactCtaSection` CTA, swapped in for the bespoke `WorkCta`) uses an AA-safe deeper **`#c43a00`** (~5.3:1 on white), overridden locally via a `--color-accent`/`--cta-*` wrapper — the **global terracotta `#C2410C` token is unchanged** (owner chose sunset-only scope; `#ff6b35` fails AA as text/fill so it stays decorative-only). **Audit fixes:** the four product mocks were on `bg-canvas` (#0a0a0a) but authored for light surfaces (active row + labels dark-on-dark) → moved to `bg-bg`; Results dark-band eyebrow/labels bumped to white/55–60 for AA; demo video compressed 38MB→836KB + poster + `preload=none` + IntersectionObserver play/pause (`work-demo-video.tsx`); hero back-link padded to a ≥44px tap target. impeccable detector clean; audit 14/20→improved, critique 31/40, both anti-patterns PASS. Other 3 case studies byte-identical (legacy path). | Establishes the warm-accent ramp pattern (bright decorative glow + AA-safe functional shade) and the case-study "show the architecture" pipeline idiom, while honouring WCAG AA (no bright-orange text/fills) and keeping the site-wide accent token untouched. |
| 2026-06-09 | **SunsetML visual enrichment — product-mock vignettes + cards-free Results + one motion moment (impeccable overdrive/layout).** Owner feedback: "What Metaborong Built" read like a document and the Results card grid was disliked. Replaced both on the **sunset path only** (other 3 keep shared `SolutionFeatures`/`ResultsBand`). New `components/work/`: `sunset-built.tsx` renders the locked feature copy as **alternating product-mock vignette rows** — each feature paired with a tokenized CSS mock of the real SunsetML feature (animated `sunset-promptbar.tsx`, model-switch / adaptive / collab mocks in `sunset-mocks.tsx`), top-aligned to the heading, sides alternating, stacking on mobile. `sunset-results.tsx` replaces the card grid with **oversized hairline-separated figures** (350 / 75 / 4 / 2026, 4-up desktop → 2×2 mobile) + qualitative outcomes as dotted lines (parsed from the md so prose stays single-sourced). **One signature motion moment** (overdrive "focus not excess"): the PromptBar mock is a scroll-triggered, IntersectionObserver-driven sequence (select → Rewrite pill → word-by-word reveal), `prefers-reduced-motion` + pre-JS render the resolved static frame; pure transform/opacity, `<style>`-scoped caret keyframe (no globals.css touch). Mocks are tokenized (brand/dark/border utilities; no raw hex, imagery-free per the dev-tool brand exception). find-skills checked — nothing beat the installed `svg-animations`/`frontend-design`. Two browser-verified craft iterations (top-align rows, taller collab mock + live-presence footer to balance row 4). `tsc`/lint clean; verified desktop+390px, no overflow; magic/orbitx/sedax byte-identical (legacy path, 200). | Kills the document feel the owner flagged: the section now *shows the app* (a writing tool demonstrating its own PromptBar, model switcher, focus mode, live collaboration) instead of describing it, and the Results moment reads as confident editorial proof rather than a SaaS card grid — one earned animation, restraint everywhere else, and zero disturbance to the case studies still awaiting their pass. |
| 2026-06-09 | **SunsetML — Technical Approach pipeline recolored to the warm palette + spine terminates at the last node.** Owner call: the routing-pipeline diagram now uses the sunset warm accent `#c43a00` everywhere (node labels, node circles, the signal pulse, the active model chip) instead of brand blue `#296ff0` — the section reads as one warm system, not a blue island. **Spine fix:** replaced the single full-height vertical line (which ran past the 03 · Model providers circle, down beside the model chips) with per-segment connectors drawn only *between* consecutive circles, so the line terminates exactly at the last circle (verified via `/agent-browser`: connector bottoms land on the next circle center, node 03 has none). Connector height is `calc(100% + 28px)` (node height + the 28px flex gap), width-independent so it holds across breakpoints and chip wrapping. **Alignment fix:** node circles were 6px above their label centers; bumped `top` so each circle centers on its node label (`delta: 0`, measured). **Motion removed (owner call):** the signal pulse that rode the connectors was dropped — the pipeline is now fully static (the spine + circles read clearly on their own; the animation wasn't reading correctly). | The pipeline now matches the rest of the warm sunset surface and the line is a precise circle-to-circle connector rather than an over-running rule — verified by direct in-browser measurement, not eyeballing. |
| 2026-06-09 | **SunsetML — "In short" direct answer recut as a framed answer panel (impeccable live, V3 of 3).** Owner flagged the 2-col eyebrow+prose "In short" block as too generic / document-like. Via `/impeccable live` (polish action, 3 variants: editorial statement / spec-manifest list / framed panel), owner picked the **framed answer panel**: a single hairline-bordered `bg-bg-subtle` card (rounded-[16px]) with a short warm `#c43a00` top rule, the mono warm eyebrow `In short · What is SunsetML?`, the definition as a bold Switzer statement, a supporting line, and a row of four capability pills. Param baked on accept: `fill: subtle` (chose the subtle tint over plain/dot-grid), `active: false`. **Repetition fix:** the pills were the four model names, which already appear as chips in the Technical Approach pipeline — swapped to the four **capabilities** (Contextual PromptBar / Live model switching / Real-time collaboration / Adaptive editor) so the model names live only in the Tech section. This is the one intentional "boxed" surface on the sunset path (the rest stays boxless); it reads as a deliberate answer card, not a generic card grid. `tsc` clean. | A direct-answer block earns a frame: the citable AEO answer now reads as a contained, scannable unit instead of loose prose, while the capability pills summarize the platform without duplicating the model chips downstream. |
| 2026-06-09 | **SunsetML — gate-clearing pass (copy tighten + impeccable audit/critique + SEO/AEO/E-E-A-T to passing).** Re-ran three independent auditors against the live page; applied each one's minimal fix set so all gates now PASS: **impeccable audit 14→18/20**, **critique 31→37/40**, **SEO 90 / AEO 82→92 / E-E-A-T 6.5→9**. (1) **Copy:** tightened the two run-on "What Metaborong Built" vignette bodies (PromptBar + Real-Time Collaboration) into short self-contained sentences, de-slopped the Built intro ("keep writers in flow, hold their focus, strip friction"), split a Direct-Answer em-dash run; all locked facts preserved, no fabrication. (2) **AEO blocker:** the `/work/[slug]/raw.md` route was dumping the source's editorial scaffold (page title, meta description, **raw keyword lists**) into the LLM-ingested export — reads as keyword-stuffing and capped AEO at 82; route now strips everything before the first `## ` heading (benefits all 4 slugs), and the stray `01` ordinal removed from `sunset.md`. (3) **E-E-A-T:** added a **visible byline + publish date** to the hero (`By Metaborong, engineering partner and equity co-founder · Published June 2026`), gated on `meta.datePublished` so legacy slugs stay byte-identical. (4) **WCAG AA contrast:** extended the sunset warm-functional **`#c43a00`** (~5.3:1) to all light-bg section eyebrows + the Built numerals + the Related-services eyebrows (was failing `text-brand/45`=1.88, `text-brand/70`=2.79, and `#296ff0`@4.24–4.36 on off-white) — unifies the eyebrow system warm (resolves the critique H4 inconsistency) and clears AA; hero meta labels `white/30`→`white/55`; mock text `text-gray-light`(2.85)→`text-gray`(5.66); decorative hero `<video>` `aria-hidden`, demo `<video>` `aria-label`; false-affordance `cursor-pointer` dropped from the demo frame when a real video plays. (5) **Motion:** PromptBar now plays **once and holds** (removed the recursive `cycle()` re-arm) — re-arms only on scroll-away/return; honours the page's own "focus" thesis. **Known/deferred:** the shared hero **category** eyebrow stays `text-brand` `#296ff0` (4.36:1 on near-black) — the systemic brand-token contrast call that spans all case studies + services, owner's decision, not changed unilaterally in sunset scope. Other 3 case studies byte-identical (legacy path, 200); `tsc` clean; detector `[]`. | Closes the gap between "shipped" and "passes the bar": the page now leads every section with a citable answer, exposes machine-clean GEO + visible authorship for AI engines, and meets WCAG AA on every text element in the sunset path — while keeping the warm SunsetML identity the owner asked for (one unified warm accent, decorative `#ff6b35` glow untouched) and leaving the systemic brand-token contrast question as an explicit owner decision. |
| 2026-06-09 | **`/work/magic` migrated to the AEO case-study grammar (MAGIC by Omagic AI) — second study on the pattern.** Brought Magic to full SunsetML parity via a per-slug `MagicCaseStudy` branch in `app/work/[slug]/page.tsx`: Hero → **"In short" framed answer panel** (`What is MAGIC?`, AEO ≤150w, `#6d28d9` accent, capability pills: Image-to-video / Controlled generation / AI packshots / Multi-platform output) → demo video → **numbered "What Metaborong Built" vignette spine** → **Technical Approach generation pipeline** → **qualitative Results band** → shared `FaqSection` → Related-services hairline list → CTA. Decision: **build Magic bespoke first, extract the shared template later** (owner call — two real instances before abstracting). New bespoke, non-card, tokenized vignettes in `components/work/` (matching Sunset's craft): `magic-pipeline.tsx` (the one signature motion moment — a scroll-triggered, reduced-motion-safe **image→video** filmstrip that renders frame-by-frame then holds), and `magic-mocks.tsx` (controlled-generation scene grid, packshot aspect grid, multi-platform fan-out strip); `magic-built.tsx` (vignette spine), `magic-tech.tsx` (Source image → Controlled generation → Cloud GPU render → Multi-format output spine, constraint chips on the generation node, output chips on the terminal node), `magic-results.tsx` (**no figure band** — MAGIC has no published metrics, so Results leads with the build credential + qualitative outcomes, parsed from the md; bold-led first bullet). **Per-slug accent ramp** (mirrors sunset's `#ff6b35`/`#c43a00`): decorative glow `#a855f7` kept; functional eyebrows/CTA/spine use AA-safe violet **`#6d28d9`** (measured 7.10:1 on white, 6.64:1 on bg-subtle) via the local `--color-accent`/`--cta-*` wrapper + a `--rs-accent` var on the related-services list; global tokens untouched. **Generalized (kept Sunset byte-identical, verified):** `buildSunsetJsonLd`→`buildWorkJsonLd` (slug-agnostic, new `meta.appName`/`appCategory` fields), `SunsetRelatedServices`→`WorkRelatedServices({title,accent})`, `resolveSunsetRelated`→`resolveRelated(slugs)`, and `WorkDemoVideo` now takes a `label` (fixes a P1: the shared video hardcoded `aria-label="SunsetML product demo"`, which had been leaking onto Magic). Copy ran the full A3 chain (keyword-research → landing-page-writer → 3 parallel auditors) to passing with **zero fabrication** — Results qualitative, byline "**engineering partner**" (not co-founder), Omagic AI linked to `omagic.ai`, real 2025 year: **content 88 / E-E-A-T 8.3 / GEO 91**. impeccable: detector `[]`, **audit 19/20**, **critique 37/40**, anti-patterns PASS. Two provided videos optimized with ffmpeg (logo 480KB→21KB, teaser 5.2MB→427KB @ CRF28 + 32KB poster, h264/yuv420p/faststart). `tsc`/lint clean (pre-existing warnings only); orbitx/sedax byte-identical (legacy path). **Deferred:** the shared `WorkHero` animated-logo lacks a reduced-motion gate + uses eager preload (decorative, aria-hidden; best fixed in the all-4 template-extraction turn). | Proves the AEO case-study grammar generalizes beyond Sunset to a structurally different product (a generative-media platform, not a writing tool) while keeping the bespoke "show the system, not a card grid" signature: four custom CSS/SVG vignettes, an architectural generation-pipeline spine, and a confident metric-free Results band — and surfaces the first reusable seams (`buildWorkJsonLd`, `WorkRelatedServices`, `resolveRelated`, `WorkDemoVideo` label) for the eventual shared template, without disturbing the studies still on the legacy path. |
| 2026-06-09 | **`/work/*` hero "All Work" back-link polished to an editorial track-line (impeccable polish).** Owner asked to better the back-link visually. Swapped the raw `←` glyph for lucide `ArrowLeft` (matches the design-system `BackButton`), lifted idle contrast `white/50`→`white/65`, and added a hairline **track-line that draws in under the label on hover** (`scaleX` 0→1, transform-only, ease-out-quint; arrow rides 3px left). Tap target padded to 45px (≥44 AA). Shared `WorkHero`, so it applies to all four case studies. `tsc`/lint clean. | The back-link reads as a deliberate, on-brand affordance instead of plain document text, with a restrained signature hover, consistent with the site's lucide iconography. |
| 2026-06-10 | **`/work/orbitx` + `/work/sedax` migrated to the AEO case-study grammar (3rd + 4th studies); shared template extracted across the Web3 pair.** Both Web3 case studies brought to full Sunset/Magic parity via per-slug `OrbitxCaseStudy` / `SedaxCaseStudy` branches. **OrbitX** (stablecoin banking on Coinbase Base): bespoke non-card vignettes for upgradeable UUPS proxy, timelock governance + RBAC, USDC treasury (Aave/Morpho), and an **animated escrow milestone-release** (`orbitx-escrow.tsx`, the signature motion moment); Technical Approach generation/banking spine. **SEDAX** (ZKP eKYC): an **animated Zero-Knowledge Proof verification** (`sedax-zkp.tsx`, "prove over-18 with no DOB shared"), plus self-sovereign-identity wallet, on-chain trust ledger, and AI fraud-detection vignettes. **Template extraction (the originally-deferred move):** the chrome is now three generic, data-driven components used by both — `work-built.tsx` (vignette spine, `{intro, features, mocks, accent}`), `work-tech.tsx` (`{intro, nodes, accent}` with optional per-node constraint/output chips), `work-results.tsx` (`{results, glow}`, qualitative dark band that renders inline **bold** + `[text](url)` links). Per-study only the 4 vignette mocks + the tech-node data + the Direct Answer JSX remain bespoke. **Per-slug AA-safe accent ramps** (same pattern as sunset `#ff6b35`/`#c43a00`, magic `#a855f7`/`#6d28d9`): OrbitX decorative `#22d3ee` + functional **`#0e7490`** teal (4.66:1 on white); SEDAX decorative `#10b981` + functional **`#047857`** emerald (5.48:1); functional shade on light surfaces, bright glow only on the dark Results band (link contrast 7.8–10.7:1 on `#0a0a0a`); globals untouched. **Byline bug fixed:** the shared `WorkHero` hardcoded "engineering partner and equity co-founder" (Sunset's), which was silently wrong on Magic and would be wrong here — now driven by `meta.bylineRole` (sunset = co-founder, magic = engineering partner, **orbitx/sedax = development partner**); Sunset render unchanged, **Magic byline corrected**. Copy ran the full A3 chain with **zero fabrication** (qualitative Results, real `orbitxpay.com` / `sedax.in` links, real years 2025/2026): OrbitX content 88 / E-E-A-T 8.5 / GEO 89, SEDAX 89 / 8.0 / 90. impeccable: detector `[]`, OrbitX audit 19/20 + critique 37/40, SEDAX audit 18/20 + critique 37/40, anti-patterns PASS. Videos optimized with ffmpeg → `public/works/{orbitx,sedax}/` (orbitx logo 39KB + demo 300KB, sedax logo 28KB + demo 582KB, both + posters; h264/yuv420p/faststart); `blendLogo` screen-blend applied to both. `resolveRelated` generalized to take a pillar id (orbitx/sedax → published `/services/web3/*` leaves). `tsc`/lint clean (pre-existing only). **Known/deferred:** SEDAX's provided demo video is a blue brand-logo loop that clashes with the emerald system (critique P1) — left as the real client asset pending owner decision; the shared `WorkHero` animated-logo reduced-motion gate + eager preload still deferred to a global pass. | Proves the AEO grammar and the now-extracted `Work*` template generalize across four structurally different products (AI writing, generative media, stablecoin banking, ZK identity) while each keeps a distinct, on-brand accent and bespoke "show the system" vignettes — and fixes a real byline misstatement that had shipped on Magic. The Web3 pair establishes the data-driven template the remaining studies (and any future case study) can adopt by supplying copy + four mocks + node data, not a new render branch from scratch. |
| 2026-05-27 | **Work-Preview rebuilt as a single-viewport "production status board."** The 2026-05-24 hairline-seam 4-up card grid (+ its `<lg` drag-scroll snap lane) read as a flat "strip" — the textbook identical-card-grid slop. Replaced with a bordered **status-board manifest**: a `PRODUCTION` / `● {n} live` header bar over hairline-divided project rows (no per-card grid). At lg+ each row is a 3-col grid (logo / pillar-mono category + Switzer title / `READ CASE STUDY →`) inside a `min-h-[calc(100svh-56px)]` centered flex frame (`lg:py-[clamp(24px,4svh,72px)]!` + `[&>div]:w-full`) — single-viewport, measured fit 712/744/844 vs frame at 1024/1280/1440. Below lg the **same `<a>` rows** reflow to stacked cards in normal flow (one DOM tree, no duplicate markup; mirrors Comparison/Why-Us responsive idiom). All UX copy + project logos preserved verbatim. The live token lives **once** in the header (owner call — per-row repetition was noise); the dot is brand blue (no untokened success-green). Removed the swipe-hint arrows, horizontal lane, and `data-lenis-prevent`. `tsc`/lint clean (only the pre-existing `/#contact` `<a>` note, identical in HEAD); verified live + zero horizontal overflow at 390/414/768/1024/1280/1440. | Breaks the identical-card-grid slop the owner flagged as "too simple," ties the visual directly to the existing "Live products… engineered for production" lede via our JetBrains-mono engineering language, and adopts the proven single-viewport + responsive-reflow idiom so it scales cleanly across every device. |
