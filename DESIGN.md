...
- **Section primitive:** `components/ui/section.tsx` — wraps every section.
  - Vertical padding: `py-[48px] md:py-[64px] lg:py-[72px]`.
...
- **Founders card hover** (`founders.tsx`, Session 19): on hover/`focus-within`,
  blueprint dashed ticks + inset frame tint to `--color-brand`, portrait zooms
  `scale(1.04)` inside its `overflow-hidden` frame, tile lifts `-3px`. 250ms
  (zoom 400ms); transform + lift are `motion-safe:`-gated.
- **Comparison row-reveal** (`comparison-table.tsx`, Session 19): IO-gated staggered
  table cascade — rows fade `opacity 0→1` + `translateY(10px→0)` at `i*55ms`, 450ms
  out-expo. Client island; short-circuits to visible under reduced-motion.
...
| File                  | Variants                                     | Notes                                                                                              |
|-----------------------|----------------------------------------------|----------------------------------------------------------------------------------------------------|
| `section.tsx`         | bg: default/subtle/dark; mw: wide/xwide/narrow/prose; `label` | Wraps every section. Auto-Reveal. `label` renders the canonical seam-band (`border-y`, left mono-uppercase title) — the single section-label device (Session 19); replaces per-section eyebrow pills. |
...
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| 2026-05-21 | **Session 19 — whole-page beautification + motion pass (no copy/structure changes).** (1) **Seam-band `SectionLabel` system:** `<Section label>` renders a full-width `border-y` band with a left mono-uppercase title at each section's top edge, replacing five divergent eyebrow treatments with one canonical device (emulates supermemory.ai section seams). Applied to Problem/Services/Why-Us/Work/Testimonials/Founders/Comparison/FAQ/Contact-CTA; Trust-bar keeps its marquee band, Hero stays label-less. Section content padding 48/64/72 → **56/72/88** (whitespace rhythm). (2) **New signature moments:** Founders card hover (blueprint-tick brand tint + portrait `scale(1.04)` zoom + `-3px` lift, 250ms, motion-safe, keyboard via `focus-within`); Comparison staggered row-reveal (IO-gated, `i*55ms`, opacity+translateY, 450ms out-expo, client island `comparison-table.tsx`); Work-Preview pillar-keyed ghost-wordmark monogram tiles + DESIGN.md-spec card hover + mobile peek lane affordance. (3) **Founder mobile image fix:** cards `100vw-32px` → `74vw max-w-300` so portraits shrink + peek. (4) Removed undocumented infinite-pulse swipe arrows (Founders + Work-Preview); peek replaces them. (5) Native smooth-scroll given a `prefers-reduced-motion` guard (no Lenis/GSAP/scroll-snap — user-approved). (6) Token fixes: `eyebrow.tsx` + work-preview raw hex → `--color-ai`/`--color-accent`/`--color-brand`. Deviations 1–8 in `docs/superpowers/specs/2026-05-21-session-19-polish-audit.md`. | Front-loaded motion gradient evened out with restrained tail choreography; page now reads as one continuous document with consistent storytelling labels, sharp seams, and consistent whitespace. CLS held at 0.0 (all motion transform/opacity); `tsc` clean; QA 1440/375. |
