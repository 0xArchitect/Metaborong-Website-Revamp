...
- **Premium finish via `/impeccable layout` + `/impeccable polish` (in tandem, user-directed).** Layout lens: dropped the bordered pill so the eyebrow aligns flush-left with the H1 (true vertical rhythm; the box had inset it). Polish lens: a static 1px brand hairline replaces the dot — the nav's ledger `<Divider>` posture (square, flat, no motion, single accent), Bauhaus-restrained, avoids the rejected terminal/badge directions. Tracking refined `0.12em → 0.14em` for premium uppercase rhythm.
