...
- **Copy.** Product Studio headline changed to **`End-to-end product engineering`** (homepage only; the prior framing still present on `/services` pages + SEO copy — flagged for a separate A3 copy pass). Centered intro lede dropped (equivalent framing retained in the FAQ JSON-LD).
