...
class CanvasTxt {
  ...
  constructor(txt: string, { fontSize = 200, fontFamily = 'Space Grotesk', color = '#fdf9f3', imageSrc }: CanvasTxtOptions = {}) {
    this.canvas = document.createElement('canvas');
    this.context = this.canvas.getContext('2d');
    this.txt = txt;
    ...
    // Make it very bold so it looks like a thick solid logo shape
  ...
  async load() {
    if (this.imageSrc) {
      return new Promise<void>((resolve, reject) => {
        ...
        img.onerror = () => {
          console.warn(`Failed to load image: ${this.imageSrc}. Falling back to text.`);
          resolve();
        };
  ...
  resize() {
    if (this.context) {
      if (this.imageObj) {
        const aspect = this.imageObj.width / this.imageObj.height;
        this.canvas.height = this.fontSize;
        this.canvas.width = this.fontSize * aspect;
      } else {
  ...
  render() {
    if (this.context) {
      ...
      if (this.imageObj) {
        ...
        // Colorize the image to the specified textColor
...
class CanvAscii {
  ...
  constructor(
    { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, imageSrc, charset }: CanvAsciiOptions,
  ...
  async setMesh() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        // Hardcode to 0 to disable all wave distortion completely
  ...
  render() {
    ...
    // Keep the logo exactly straight
  ...
  useEffect(() => {
    ...
    const createAndInit = async (container: HTMLDivElement, w: number, h: number) => {
      const instance = new CanvAscii(
        { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, imageSrc, charset },
        container,
        w,
        h
      );
