...
import { MagicPipeline } from './magic-pipeline'
import { ControlledGenerationMock, PackshotGridMock, MultiPlatformMock } from './magic-mocks'

// "What Metaborong Built" — alternating product-mock vignette rows. Each
// locked-copy feature is paired with a stylized mock of the real MAGIC
// capability (mock and copy swap sides each row) so the section shows the
// platform instead of reading like a document. The mock order matches the
// `### ` feature order in content/work/magic.md.

const MOCKS = [
  <MagicPipeline key="v" />,
  <ControlledGenerationMock key="c" />,
  <PackshotGridMock key="p" />,
  <MultiPlatformMock key="m" />,
]
...
export function MagicBuilt({ intro, features }: { intro: string; features: Feat[] }) {
  return (
    <Section bg="default" maxWidth="xwide" className="py-[44px] sm:py-[64px] lg:py-[88px] border-t border-border">
      <div className="mb-[36px] max-w-[680px] sm:mb-[48px] lg:mb-[64px]">
        <h2 className="mb-[16px] font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.14em] text-[#6d28d9]">What Metaborong Built</h2>
        {paras(intro).map((p, i) => (
          ...
          return (
            <div key={i} className="grid items-center gap-[20px] sm:gap-[24px] lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-[56px] xl:gap-[72px]">
              ...
              <div className={flip ? 'lg:order-1' : ''}>
                <span className="font-mono text-[12px] font-bold tracking-[0.12em] text-[#6d28d9]">{String(i + 1).padStart(2, '0')}</span>
