...
const BLUE_R    = 0.019
const NODE_R    = 0.030
const AUTO_SPEED = 0.058  // rad/s — equivalent to OrbitControls autoRotateSpeed ≈ 0.55
...
function buildBlueMesh(): THREE.InstancedMesh {
  ...
  const mesh  = new THREE.InstancedMesh(geo, mat, N_TOTAL - N_ORANGE)
...
function buildEdgeLines(): THREE.LineSegments {
  ...
  const posArr = new Float32Array(pairs.length * 3)
  for (let i = 0; i < pairs.length; i++) _worldPts[pairs[i]].toArray(posArr, i * 3)
  const geo = new THREE.BufferGeometry()
  geo.setAttribute('position', new THREE.BufferAttribute(posArr, 3))
  return new THREE.LineSegments(
    geo,
    new THREE.LineBasicMaterial({ color: '#204AF8', transparent: true, opacity: 0.16 })
  )
}

// ── CSS keyframes (injected once, isolated from Tailwind purge) ───────────────
...
// ── Custom orbit controller (replaces OrbitControls) ─────────────────────────
// Proper click-vs-drag detection: < 5px movement = click, lets r3f onClick fire.
// > 5px movement = drag, applies rotation velocity with damping back to auto-speed.
...
function useOrbit(groupRef: React.RefObject<THREE.Group | null>) {
  ...
  useFrame((_, delta) => {
    ...
    if (!d.current.active) {
      // Smoothly damp back to auto-rotation speed after a drag
    ...
    groupRef.current.rotation.x  = Math.max(
      -0.28, Math.min(0.28, groupRef.current.rotation.x + velX.current)
    )
  })
}
...
interface LabelState {
  ...
  openLeft:  boolean   // true when node is on right side of screen → label opens left
}
...
function FloatingLabel({ state, onExpire }: { state: LabelState; onExpire: () => void }) {
  ...
  // Anchor logic:
  //   Right hemisphere (openLeft): bottom-RIGHT corner sits above node → label extends left
  //   Left  hemisphere           : bottom-LEFT  corner sits above node → label extends right
  // translateY(-100%) lifts the label so its bottom edge is at the node's projected position.
  const wrapTransform = state.openLeft
    ? 'translateX(-100%) translateY(-100%)'  // right hemi
    : 'translateX(0%)    translateY(-100%)'  // left hemi
  ...
  return (
    ...
    >
      {/* Outer wrapper is purely positional — no animation applied here.
          Inner div carries the scale+fade entry so the label materialises
          in place without any translate slide. */}
      <div style={{
        position:     'relative',
        ...
        <span style={{
          ...
          animation:     '_orbScn 2.2s linear infinite',
          pointerEvents: 'none',
        }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, position: 'relative' }}>
          ...
          <span style={{
            ...
            color,
            fontWeight: 700,
            animation:  '_orbCur .8s step-end infinite',
            marginLeft: 1,
          }}>_</span>
        </div>
      </div>
...
// ── Service node (color-coded, clickable) ─────────────────────────────────────
...
function ServiceNode({
  ...
  return (
    <group position={pos}>
      {/* Glow halo (larger hit surface helps hover detection) */}
      <mesh onPointerEnter={onEnter} onPointerLeave={onLeave}>
        <sphereGeometry args={[NODE_R * 2.4, 8, 6]} />
        <meshBasicMaterial color={color} transparent opacity={0.10} />
      </mesh>
      {/* Visible core */}
      <mesh onPointerEnter={onEnter} onPointerLeave={onLeave}>
        <sphereGeometry args={[NODE_R, 10, 8]} />
...
// ── Controller component (must be inside Canvas) ──────────────────────────────
  ...
  return (
    <>
      ...
      <group ref={groupRef}>
        ...
        {label && (
          <FloatingLabel key={label.id} state={label} onExpire={clearLabel} />
        )}
      </group>
    </>
  )
